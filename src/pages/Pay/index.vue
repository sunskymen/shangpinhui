...
<script>
// 二维码插件
import { clearInterval } from 'lodash/_freeGlobal'
  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, '微信支付', {
        ...
        cancelButtonText: '不想支付了',
        confirmButtonText: '支付成功',
        showClose: false,
        beforeClose: (type, instance, done) => {
          // type区分点击的类型,instance组件实例,done关闭弹出框的方法
          if (type == 'cancel') {
            alert('为啥不支付')
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          ...
          if (res.code === 200) {
            // 支付成功, 清除定时器 , 保存支付状态码, 关闭弹窗 , 跳转支付成功页面
            clearInterval('this.timer')
            this.timer = null
            this.code = res.code
            this.$msgbox.close()
