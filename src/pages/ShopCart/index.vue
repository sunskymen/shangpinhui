      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="removeGoods(cart.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    // 修改购物车产品数量  disNum 传递给服务器的是增量 需要进行计算
    ...
    async removeGoods(skuId) {
      try {
        await this.$store.dispatch('removeGoods', skuId)
        this.getData()
      } catch (error) {
        alert(error.message)
      }
    ...
    async updateChecked(skuId, checked) {
      try {
        await this.$store.dispatch('updateCheckedById', { skuId, isChecked: checked == true ? 1 : 0 })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
